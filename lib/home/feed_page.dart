...
class _FeedPageState extends State<FeedPage> {
  ...
  String _dateFilterType =
      'All Time'; // 'All Time', 'Last Week', 'Last Month', 'Custom Range'
  ...
  Set<String> _upvotedIssueIds =
      {}; // To store IDs of issues upvoted by the current user
    ...
    try {
      ...
      final position = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy
            .medium, // Changed to medium for faster initial load
      );
    ...
    try {
        ...
        if (userId != null) {
          try {
            ...
            filteredIssues[i] = Map<String, dynamic>.from(issue)
              ..['reporter_first_name'] =
                  userResponse['first_name'] as String? ?? 'Anonymous'
              ..['reporter_last_name'] =
                  userResponse['last_name'] as String? ?? '';
      ...
      if (!_selectedCategories.contains('All') &&
        ...
        filteredIssues = filteredIssues
            .where((issue) => _selectedCategories.contains(issue['category']))
            .toList();
      }
      ...
      if (_startDate != null && _endDate != null) {
        filteredIssues = filteredIssues.where((issue) {
          final issueDate = DateTime.parse(
            issue['created_at'] ?? '1970-01-01T00:00:00Z',
          );
          return issueDate.isAfter(_startDate!) &&
              issueDate.isBefore(_endDate!);
        }).toList();
      }
      ...
      if (_showPrioritized) {
        ...
        filteredIssues.sort((a, b) {
          final dateA = DateTime.parse(
            a['created_at'] ?? '1970-01-01T00:00:00Z',
          );
          final dateB = DateTime.parse(
            b['created_at'] ?? '1970-01-01T00:00:00Z',
          );
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error fetching issues: $e')));
      }
    }
  }
    ...
    return CustomScrollView(
      slivers: [
        CupertinoSliverNavigationBar(
          heroTag: 'feed_nav_bar', // Unique hero tag to avoid conflicts
          alwaysShowMiddle: true,

          largeTitle: Text('Feed', style: AppTextStyles.headlineLarge),
          trailing: Row(
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_dateFilterType == 'Custom Range' &&
                      _startDate != null &&
                      _endDate != null)
                    Padding(
          ...
          builder: (BuildContext context, StateSetter setModalState) {
            return Container(
              ...
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Sort Issues', style: AppTextStyles.titleLarge),
                  SizedBox(height: 16),
                  RadioListTile<bool>(
  ...
  Widget _buildIssueCardFromData(dynamic issue) {
    final issueId = issue['id'] as String? ?? '';
    final issueNumber = issueId.substring(
      0,
      8,
    ); // First 8 digits of UUID as issue number
    ...
    final reporterName = reporterFirstName == 'Anonymous'
        ? 'Anonymous'
        : '$reporterFirstName $reporterLastName';
    final reporterId =
        issue['user_id'] as String?; // Get the reporter's user ID

    final bool hasUpvoted = _upvotedIssueIds.contains(issueId);
    final bool isReporter =
        _currentUserId != null && _currentUserId == reporterId;
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('$upvotes upvotes', style: AppTextStyles.bodySmall),
                  ElevatedButton(
                    onPressed: isReporter || hasUpvoted
                        ? null
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('$issue - $location', style: AppTextStyles.titleSmall),
                  SizedBox(height: 4),
                  Text(
                    '$upvotes↑',
                    style: AppTextStyles.bodySmall.copyWith(color: Colors.grey),
